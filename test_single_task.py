...
def run_single_task():
    ...
    """
    task = Task.get_tabular_tasks(n_train=256, al_batch_sizes=[256] * 16, ds_names=['ct'])[0]
    task_split = TaskSplit(task, id=6)
    sigma = 1e-2
    compute_eff_dim = True
    kwargs = dict(maxdet_sigma=sigma, compute_eff_dim=compute_eff_dim, # allow_float64=True,
    ...
    results = trainer(task_split, device=get_devices()[0])
    print(results)
